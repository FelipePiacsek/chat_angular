...
def directive_quotation_mt(args):
	...
	total_value = float(per_month_beneficiary_value) * int (number_of_beneficiaries) + float(config['QUOTATION_DIRECTIVE_MT_BASE_VALUE'])
	...
	obj['parameters'] = {'directive_type':'quotation',
						 ...
						 'proposal': {'currency': args.get('currency','$'),
						 			  ...
						 			  'base_value': str(config['QUOTATION_DIRECTIVE_MT_BASE_VALUE']) ,
						 ...
						 'company': {'name':args.get('company_name',''),
						 			 'picture':args.get('company_picture', config['QUOTATION_DIRECTIVE_MT_DEFAULT_COMPANY_PICTURE'])
						 			},
